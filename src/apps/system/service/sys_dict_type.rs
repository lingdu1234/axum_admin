...
pub async fn ddelete(
    ...
    s = s.filter(sys_dict_type::Column::DictId.is_in(delete_req.dict_ids));
...
pub async fn get_by_id(
    ...
    if let Some(x) = search_req.dict_id {
        s = s.filter(sys_dict_type::Column::DictId.eq(x));
    } else {
...
pub async fn get_all_dict_type(
    ...
    let s = SysDictType::find()
        ...
        .order_by(sys_dict_type::Column::DictId, Order::Asc)
        .all(db)
        .await?;
