...
// pub async fn get_enforcer2(is_init: bool) -> &'static Arc<Mutex<Enforcer>> {
//     static CASBIN: OnceCell<Arc<Mutex<Enforcer>>> = OnceCell::const_new();
//     CASBIN
//         .get_or_init(|| async {
//             println!("CasbinService 开始初始化了………………………………………………………………………………………………2………………………………………………………………………………………………………………………");
//             let db = DB.get_or_init(db_conn).await;
//             let m = DefaultModel::from_file(&CFG.casbin.model_file)
//                 .await
//                 .unwrap();
//             let adpt = SeaOrmAdapter::new_with_pool(db.clone(),is_init).await.unwrap();
//             let mut e = Enforcer::new(m, adpt).await.unwrap();
//             e.enable_log(true);
//             Arc::new(Mutex::new(e))
//         })
