...
/// page_params 分页参数
/// db 数据库连接 使用db.0
pub async fn get_sort_list(db: &DatabaseConnection, page_params: PageParams, req: SysMenuSearchReq) -> Result<ListData<SysMenuModel>> {
    ...
    C: TransactionTrait + ConnectionTrait,
{
    ...
    let active_model = sys_menu::ActiveModel {
        ...
        icon: Set(req.icon.unwrap_or_default()),
        ...
        path: Set(req.path.unwrap_or_default()),
        component: Set(req.component.unwrap_or_default()),
...
// 多写个方法，少返回点数据
pub fn get_menu_data2(menus: Vec<SysMenuModel>) -> Vec<SysMenuTreeAll> {
