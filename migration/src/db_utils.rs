...
where
    E: EntityTrait,
{
    db.execute(builder.build(schema.create_table_from_entity(e).to_owned().if_not_exists())).await?;
    println!("创建表格成功:{}", e.table_name());
    Ok(())
}
    ...
    T: 'static + Iden + EntityTrait,
{
    ...
    println!("成功创建索引,表格:{},索引名:{},索引列:{:?}", t.table_name(), name, cols_name);
    Ok(())
}
...
pub async fn init_data(manager: &SchemaManager<'_>, migration_name: &str) -> Result<(), DbErr> {
    ...
    while let Some(res) = entries.next().await {
        ...
        let path = entry.path();
        let sql_vec = match get_insert_sql_string(path.clone()).await {
            Ok(v) => v,
            Err(e) => {
                println!("{:?}", e.to_string());
                return Ok(());
    ...
    while let Some(line) = buf_reader.next().await {
        match line {
            ...
            Ok(v) => {
                ...
                let vv = if v.starts_with("INSERT") || v.starts_with("insert") { v.replace("`", "") } else { v };
